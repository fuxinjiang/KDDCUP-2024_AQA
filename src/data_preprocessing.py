...
def read_data():
    raw_doc = json.load(open(osp.join(path_root,'data/AQA/pid_to_title_abs_update_filter.json')))
    raw_question = {}
...
def process_data(raw_doc, raw_question):
    ...
    if not osp.exists(osp.join(path_root, 'data/process_data')):
        os.makedirs(osp.join(path_root, 'data/process_data'))
